...
//  MOAdsView
//
//  Created by moxiaohao on 2017/3/31.
...
class MOAdsView: UIView {
    ...
    fileprivate func setupSubviews() {
        ...
        timeBtn = UIButton.init(frame: CGRect.init(x: screen_width - 70, y: 30, width: width, height: height))
        ...
        timeBtn.addTarget(self, action: #selector(MOAdsView.jumpAD), for: .touchUpInside)
    ...
    fileprivate func changeLocation() {
        ...
        case .topHorizontally:
            self.timeBtn.frame = CGRect(x:(screen_width - rect.width) / 2, y: 30, width: rect.width, height: rect.height)
            
        case .topLeftCorner:
            self.timeBtn.frame = CGRect(x:15, y: 30, width: rect.width, height: rect.height)
            
        case .topRightCorner:
            self.timeBtn.frame = CGRect(x:screen_width - 65, y: 30, width: rect.width, height: rect.height)
    ...
    public func start() {
        if #available(iOS 10.0, *) {
            ...
            timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(MOAdsView.countDown), userInfo: nil, repeats: true)
        }
