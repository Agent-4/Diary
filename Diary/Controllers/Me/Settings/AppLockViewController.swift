...
//  Mo
//
//  Created by moxiaohao on 2016/12/31.
...
class AppLockViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //去除导航栏底部黑线
        AppLockViewNavBar?.py_add(toThemeColorPool: "barTintColor")
        ...
        view_round1.layer.borderColor = green.cgColor
        ...
        view_round2.layer.borderColor = green.cgColor
        ...
        view_round3.layer.borderColor = green.cgColor
        ...
        view_round4.layer.borderColor = green.cgColor
    ...
    @IBAction func forgotPWDBtnClick(_ sender: UIBarButtonItem) {
        ...
        let okAction = UIAlertAction(title: "登录", style: .destructive, handler: {
            action in
            AVUser.logInWithUsername(inBackground: (AVUser.current()?.username)!, password: (passwordLogin.textFields?.first?.text!)!, block: {
                (user, error) in
                if user != nil {
                    ...
                    switch errors.code {
                    case 210:
                        if passwordLogin.textFields?.first?.text! == "" {
                            Drop.down("密码不能为空！", state: .color(red), duration: 2)
                        }else{Drop.down("密码错误！", state: .color(red), duration: 2)}
                    case 219:
                        Drop.down("登录失败次数超过限制，请稍候再试", state: .color(red), duration: 2)
                    default:
    ...
    func numberBtnClick(tagCount: UIButton) {
        ...
        if (view_round1.backgroundColor == nil) {
            view_round1.backgroundColor = green
        }
        else if (view_round2.backgroundColor == nil) {
            view_round1.backgroundColor = green
            view_round2.backgroundColor = green
        }
        else if (view_round3.backgroundColor == nil) {
            view_round1.backgroundColor = green
            view_round2.backgroundColor = green
            view_round3.backgroundColor = green
        }
        else if (view_round4.backgroundColor == nil) {
            view_round1.backgroundColor = green
            view_round2.backgroundColor = green
            view_round3.backgroundColor = green
            view_round4.backgroundColor = green
        }
        ...
        if (inputPWD .isEqual(truePWD) && getArray.count == 4) {
            showMessage(message: "密码正确!", backgroundColor: green, delayTime: 2)
            print("密码正确！")
            ...
            showMessage(message: "密码错误!", backgroundColor: red, delayTime: 2)
